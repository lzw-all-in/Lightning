...
Http::Http(std::string& root): _state(HTTP_PARSE_STATE::start), _root(root), _keep_alive(false)
{  
}
...
int Http::parseHttp(int fd, char* buff, size_t len) {
    ...
    // 如果是长链接需要清除状态机的状态,这一步很关键，不然会一直留在当前页面
    _clear_state();

    return _keep_alive ? HTTP_LONG_CONNECTION : HTTP_SHORT_CONNECTION;
}
